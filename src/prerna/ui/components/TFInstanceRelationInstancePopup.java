...
public class TFInstanceRelationInstancePopup extends JMenu implements MouseListener{
	...
	public TFInstanceRelationInstancePopup(String name, IPlaySheet ps, DBCMVertex [] pickedVertex)
	{
		super(name);
	...
	public void addRelations(String prefix)
	{
		...
		for(int pi = 0;pi < pickedVertex.length;pi++)
		{
			...
			while(sjw.hasNext())
			{
				...
				hash.put("SUBJECT", uri2);
				hash.put("OBJECT", predName);
				...
				if(predClassName.length() > 0 && !Utility.checkPatternInString(ignoreURI, predClassName) && (!instance && !predClassName.contains("\"")))
				{
					...
					logger.debug("Adding Relation " + predName);
					NeighborMenuItem nItem = new NeighborMenuItem(predName, nFillQuery, engine);
					nItem.addActionListener(NeighborMenuListener.getInstance());
					add(nItem);
					hash.put(predName, predName);
				}
